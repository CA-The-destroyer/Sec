...
# Protect /opt/Citrix directory from accidental deletion
import shutil

# Keep original rmtree
_original_rmtree = shutil.rmtree

def _protected_rmtree(path, *args, **kwargs):
    # Skip removal of anything under /opt/Citrix/
    if isinstance(path, str) and path.startswith('/opt/Citrix/'):
        # Log to stderr or ignore silently
        return
    return _original_rmtree(path, *args, **kwargs)

# Monkey-patch shutil.rmtree globally in this module
shutil.rmtree = _protected_rmtree
