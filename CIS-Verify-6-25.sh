...
echo "=== 1.1 Filesystem ==="
check "Separate partitions on /tmp"    "mount | grep -q \" on /tmp \""
...
check "Separate partitions on /dev/shm" "mount | grep -q \" on /dev/shm \""
...
check "no pending yum updates"          "yum check-update >/dev/null 2>&1; test \$? -eq 0"

echo
echo "=== 1.3 SELinux ==="
check "SELinux policy installed"        "rpm -q selinux-policy-targeted >/dev/null"
check "SELinux enforcing mode"          "getenforce | grep -q 'Enforcing'"
...
check "SSH FIPS ciphers"                "grep -Eq '^Ciphers .*aes256-ctr' /etc/ssh/sshd_config"
...
check "GDM idle lock"                   "gsettings get org.gnome.desktop.session idle-delay | grep -Eq '[1-9]+'"
check "no autorun overrides in GDM"     "grep -Eq 'autorun-never' /etc/gdm/custom.conf"

echo
echo "=== 2.1 Server Services ==="
for svc in cups rpcbind avahi samba; do
  check "Service $svc disabled/not running" \
    "! systemctl is-enabled $svc 2>/dev/null && ! systemctl is-active $svc 2>/dev/null"
done

echo
echo "=== 2.2 Client Services ==="
check "telnet client not installed"      "! rpm -q telnet >/dev/null 2>&1"

echo
echo "=== 2.3 Chrony ==="
check "chrony installed"                 "rpm -q chrony >/dev/null"
check "chronyd enabled+running"         "systemctl is-enabled chronyd >/dev/null && systemctl is-active chronyd >/dev/null"
...
check "cron.allow contains root"        "test -f /etc/cron.allow && grep -q '^root$' /etc/cron.allow"
check "cron.deny empty or absent"       "! test -f /etc/cron.deny || grep -q '^$' /etc/cron.deny"
check "at.allow contains root"          "test -f /etc/at.allow && grep -q '^root$' /etc/at.allow"
check "at.deny empty or absent"         "! test -f /etc/at.deny || grep -q '^$' /etc/at.deny"
...
check "firewalld loopback ok"           "firewall-cmd --zone=trusted --query-interface=lo >/dev/null 2>&1"
...
check "pam,authselect,pwquality pkgs"   "rpm -q pam authselect libpwquality >/dev/null"
check "pwquality minlen≥14,difok≥4,remember≥5" \
     "grep -E 'pam_pwquality.so.*minlen=14' /etc/pam.d/system-auth && grep -E 'difok=4' /etc/pam.d/system-auth && grep -E 'remember=5' /etc/pam.d/system-auth"
...
check "aide installed"                  "rpm -q aide >/dev/null"
check "aide.conf not empty"             "grep -Eq '/.*f' /etc/aide.conf"

echo
echo "=== 6.2 Journald & Rsyslog ==="
check "journald enabled+active"         "systemctl is-enabled systemd-journald >/dev/null && systemctl is-active systemd-journald >/dev/null"
check "ForwardToSyslog=no"              "grep -E '^ForwardToSyslog=no' /etc/systemd/journald.conf"
check "Compress=force or yes"           "grep -E '^Compress=' /etc/systemd/journald.conf"
check "Storage="                        "grep -E '^Storage=' /etc/systemd/journald.conf"
check "rsyslog installed+active"        "rpm -q rsyslog >/dev/null && systemctl is-active rsyslog >/dev/null"
check "rsyslog remote host line"        "grep -Eq '@' /etc/rsyslog.conf"

echo
echo "=== 7.1 Maintenance (no-owner files) ==="
check "no-files without owner/group"    "! find / -xdev \( -nouser -o -nogroup \) | grep -q ."

echo
echo "Done.  
(Controls 4.3.2 & 4.3.3 skipped.)"
